...
def menu():
    ...
    try:
        ...
        kunci = '-'
        bergabung = Barusan 
        kadaluarsa = Unlimited
        pro = ("%s[%sPro%s]"%(O,P,O))
        upgrade = ('Upgrade To Version %s--'%(O))
        jid = ('%s[%s80000%s]'%(O,P,O))
    try:
    ...
    print('%s╠══[%s•%s] %sName : Arjuna%s'%(O,P,O,P,users))
    print('%s╠══[%s•%s] %sEmail : naajun16@gmail.com%s'%(O,P,O,P,maile))
